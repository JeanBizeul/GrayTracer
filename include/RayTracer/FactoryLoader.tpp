...
static std::string extractSharedLibName(const std::string &filename) {
    ...
    if (filename.size() < prefix.size() + suffix.size() + 1) {
        std::cerr << "Incorrect shared library file name '" << filename
                  << "': too low character count.\n"
                  << "Minimum is 'liba.so'" << std::endl;
        return "";
    }
    if (filename.substr(0, prefix.size()) != prefix) {
        std::cerr << "Cannot find prefix '" + prefix + "' in " << filename
                  << std::endl;
        return "";
    }
    if (filename.substr(filename.size() - suffix.size()) != suffix) {
        std::cerr << "Cannot find suffix '" + suffix + "' in " << filename
                  << std::endl;
        return "";
    }
    ...
    for (auto file : it) {
        if (file.is_regular_file() &&
            ...
            if (!loadFactory(file.path())) {
                std::cerr << "Shared file " << file.path().filename()
...
bool FactoryLoader::loadFactory(const std::filesystem::path &path) {
    std::cout << "[FactoryLoader] Trying to load: " << path.filename()
              << std::endl;
...
std::unique_ptr<T> FactoryLoader::create(const std::string &tag,
    ...
    std::unordered_map<std::string, std::unique_ptr<RayTracer::IFactory<T>>>
        *map = nullptr;
